...
When finding the translation, ignore the line number such as §0§, §1§ at the beginning of each sentence.
...
Strictly follow the output format and output only the line number:
 [0] : [0, 1]
 [1] : [2]
'''
...
def align_lines(file, index, source_lines, target_lines, model_name, category):
    ...
    client = OpenAI(base_url="https://openrouter.ai/api/v1", api_key = api_keys[0])
    ...
    print("finish aligning", completion)
    if completion:
        result_text = completion.choices[0].message.content.strip()
        print(result_text)

        output_dir = f"llmladderwithlinenumber/{category}/{model_name}"
...
category = "law"
# ---------------- MAIN EXECUTION ----------------
folder_path = f"../golddataset/{category}/ar"
for file in os.listdir(folder_path):
    ...
    index = 0
    align_lines(file, index, source_lines, target_lines, model_name, category)
